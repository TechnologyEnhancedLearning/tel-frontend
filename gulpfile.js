 ...
 */
function copySource() {
  return gulp.src('src/**/*').pipe(gulp.dest('dist/tel-frontend'))
}
...
function createZip() {
  return gulp
    .src(['dist/tel-frontend/**', `dist/main-${packageJson.version}.min.css`])
    .pipe(zip(`tel-frontend-${packageJson.version}.zip`))
    .pipe(gulp.dest('dist'))
}
